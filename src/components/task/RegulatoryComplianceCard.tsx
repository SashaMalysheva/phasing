...
const RegulatoryComplianceCard: React.FC<CardProps> = ({ card, onClose, onTypeChange }) => {
  ...
  if (card.status === 'crc_todo') {
    ...
    content = <>
      <button onClick={onClose} className="absolute top-2 right-2 text-gray-400 hover:text-gray-700">×</button>
      <div>RegulatoryComplianceCard — CRC In Progress</div>
      <div className="mb-2">Тип: {card.type}</div>
      <select
        className="border rounded px-2 py-1"
        value={card.type}
    ...
    content = <>
      <button onClick={onClose} className="absolute top-2 right-2 text-gray-400 hover:text-gray-700">×</button>
      <div>RegulatoryComplianceCard — AI For Review</div>
      <div className="mb-2">Тип: {card.type}</div>
      <select
        className="border rounded px-2 py-1"
        value={card.type}
    ...
    content = <>
      <button onClick={onClose} className="absolute top-2 right-2 text-gray-400 hover:text-gray-700">×</button>
      <div>RegulatoryComplianceCard — AI In Progress</div>
      <div className="mb-2">Тип: {card.type}</div>
      <select
        className="border rounded px-2 py-1"
        value={card.type}
